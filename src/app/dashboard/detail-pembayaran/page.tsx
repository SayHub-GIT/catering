...
export default function DetailPembayaran() {
  ...
  async function fetchData() {
    ...
    try {
      const [detailRes, metodeRes] = await Promise.all([
        supabase.from("detail_jenis_pembayarans").select("*, jenis_pembayarans(metode_pembayaran)").order("created_at", { ascending: false }),
        supabase.from("jenis_pembayarans").select("*")
      ]);
      ...
      if (metodeRes.data && metodeRes.data.length > 0) {
        setFormData(f => ({ ...f, id_jenis_pembayaran: metodeRes.data[0].id }));
      }
    } catch (error) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      setFormData({ ...formData, no_rek: "", tempat_bayar: "", logo: "" });
      setIsAdding(false);
      fetchData();
    } catch (error: any) {
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await supabase.from("detail_jenis_pembayarans").delete().eq("id", id);
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Detail Rekening & Pembayaran</h1>
          <p className="text-muted-foreground">Kelola nomor rekening atau e-wallet untuk setiap metode pembayaran.</p>
        </div>
        <button 
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select 
                ...
                onChange={(e) => setFormData({...formData, id_jenis_pembayaran: e.target.value})}
              >
                {metodes.map(m => <option key={m.id} value={m.id}>{m.metode_pembayaran}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm mb-1">Tempat Bayar (Bank/E-Wallet)</label>
              <input 
                type="text" 
                ...
                onChange={(e) => setFormData({...formData, tempat_bayar: e.target.value})}
              />
            </div>
            <div>
              <label className="block text-sm mb-1">No. Rekening / No. HP</label>
              <input 
                type="text" 
                required
                className="w-full p-2 border border-border rounded-lg"
            ...
            <div className="col-span-2 pt-2 flex gap-2">
              <button type="submit" className="bg-primary text-primary-foreground px-4 py-2 rounded-lg font-medium">Simpan</button>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {details.length === 0 ? (
              <tr><td colSpan={4} className="px-6 py-8 text-center text-muted-foreground">Belum ada data detail.</td></tr>
            ) : (
              details.map((d) => (
                <tr key={d.id} className="border-b border-border hover:bg-secondary/20">
                  <td className="px-6 py-4 font-bold">{d.jenis_pembayarans?.metode_pembayaran}</td>
                  <td className="px-6 py-4">{d.tempat_bayar}</td>
                  <td className="px-6 py-4 font-mono text-primary">{d.no_rek}</td>
